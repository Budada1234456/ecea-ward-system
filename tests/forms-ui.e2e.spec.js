...
test("structured forms, entity ordering and discipline tree work responsively", async ({
  ...
  try {
    ...
    const stickyColumns = await scroll.evaluate((element) => {
      ...
      const first = element.querySelector(".structured-table__row .structured-table__sticky-start").getBoundingClientRect();
      const last = element.querySelector(".structured-table__row .structured-table__sticky-end").getBoundingClientRect();
      return {
        scrolled: element.scrollLeft > 0,
        firstVisible: first.left >= bounds.left - 1 && first.right <= bounds.right + 1,
        lastVisible: last.left >= bounds.left - 1 && last.right <= bounds.right + 1,
      };
    });
    expect(stickyColumns).toEqual({ scrolled: true, firstVisible: true, lastVisible: true });
    ...
    await expect(economicPreview).toContainText("创收外汇（美元）");
