...
test("schema keeps field and PDF order aligned", () => {
  ...
  for (const group of FIELD_GROUPS) {
    ...
    assert.ok(fields.every(({ hint }) => typeof hint === "string" && hint.trim()));
...
test("primary discipline follows the first selection after normalization", () => {
  const source = [
    { name: "第一学科", code: "4806010", status: DISCIPLINE_STATUS.CONFIRMED, path: ["480", "48060", "4806010"] },
    "历史自由文本",
  ];
  const data = normalizeApplicationData({ disciplines: source });
  assert.deepEqual(derivePrimaryDiscipline(data.disciplines), data.disciplines[0]);
  assert.equal(derivePrimaryDiscipline(data.disciplines).code, "4806010");
  assert.equal(derivePrimaryDiscipline([...data.disciplines].reverse()).name, "历史自由文本");
...
test("legacy entity records receive stable unique ids before reordering", () => {
  ...
  assert.deepEqual(
    records.map(({ id }) => id),
    ["duplicate", "duplicate-1", "person-legacy-3"],
  );
  assert.deepEqual(
