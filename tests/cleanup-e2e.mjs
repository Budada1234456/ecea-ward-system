...
export default function cleanupE2e() {
  const targetUrl = new URL(
    process.env.TEST_BASE_URL || "http://127.0.0.1:4174",
  );
  if (!["127.0.0.1", "localhost", "::1"].includes(targetUrl.hostname)) {
    console.warn(
      `Skipping local database cleanup for non-local TEST_BASE_URL: ${targetUrl.origin}`,
    );
    return;
  }
  ...
  try {
    const users = db
      .prepare(`SELECT id, username FROM users WHERE ${testUserPredicate}`)
      .all(...testUserPatterns);
      ...
      .prepare(
        `SELECT id, title
         ...
         WHERE title LIKE ?
            OR title LIKE ?
            OR owner_id IN (
              SELECT id FROM users WHERE ${testUserPredicate}
            )`,
      )
      .all("浏览器验收项目-%", "表单组件验收项目-%", ...testUserPatterns);
