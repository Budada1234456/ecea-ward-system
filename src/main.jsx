...
function disciplineDisplay(item) {
  if (!item) return "";
      ...
      <table>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td colSpan="4">{disciplineDisplay(derivePrimaryDiscipline(data.disciplines))}</td>
          </tr>
          <tr>
            <td className="subhead">2</td>
            <td colSpan="4">{disciplineDisplay(data.disciplines?.[1])}</td>
          </tr>
          <tr>
            <td className="subhead">3</td>
            <td colSpan="4">{disciplineDisplay(data.disciplines?.[2])}</td>
          </tr>
          <tr>
...
function PreviewEconomicPage({ data, pageNumber }) {
  ...
  const amountFields = referenceKeys
    .map((key) =>
      getPdfFields("economicRecords").find((field) => field.key === key),
    )
    .filter(Boolean);
  const total = (key) =>
  ...
  useEffect(() => {
    ...
    const flush = () => {
      ...
      fetch(`/api/applications/${application.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ data: normalizeApplicationData(latestData.current) }),
        keepalive: true,
      }).catch(() => {});
    };
