...
export function getDisciplineOptions(
  ...
  for (const item of disciplines) {
    if (matchesDisciplineQuery(item, normalizedQuery)) matches.push(item);
...
export function getDisciplinePath(disciplines = [], code = null, index) {
  if (!code) return [];
  const byCode = index || new Map(disciplines.map((item) => [item.code, item]));
