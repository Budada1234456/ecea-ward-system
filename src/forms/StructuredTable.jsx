...
export function StructuredTable({
  ...
  const addRecord = () => {
    ...
    pendingFocus.current = `${rowIndex}:${fields[0]?.key}`;
    onChange([
      ...records,
      group
  ...
  const removeRecord = (rowIndex) => {
    ...
    if (next.length) {
      pendingFocus.current = `${Math.min(rowIndex, next.length - 1)}:${fields[0]?.key}`;
    }
    setTouched(
      (current) =>
        new Set([...current].filter((key) => !key.startsWith(`${rowIndex}:`))),
    );
    onChange(next);
  };
  const markTouched = (rowIndex, fieldKey) =>
    setTouched((current) => new Set(current).add(`${rowIndex}:${fieldKey}`));
        ...
        >
          <div className="structured-table__grid" style={{ minWidth }}>
            <div
              ...
              role="row"
            >
              {showIndex && <span className="structured-table__sticky-start" role="columnheader">序号</span>}
              {fields.map((field, index) => (
                <span role="columnheader" className={!showIndex && index === 0 ? "structured-table__sticky-start" : undefined} key={field.key}>
                  {field.required && <i aria-hidden="true">*</i>}
                  {field.label}
                </span>
              ))}
              <span className="structured-table__sticky-end" role="columnheader">操作</span>
            </div>
            {records.map((record, rowIndex) => (
              ...
              >
                ...
                {fields.map((field, fieldIndex) => {
                  ...
                  const touchKey = `${rowIndex}:${field.key}`;
                  const invalid =
                  ...
                  return (
                    ...
                    >
                      ...
                      <FieldControl
                        ...
                        onBlur={() => markTouched(rowIndex, field.key)}
                        onChange={(nextValue) =>
                          updateRecord(rowIndex, field, nextValue)
                        }
                      />
                      {field.hint && <small className="structured-table__hint">{field.hint}</small>}
